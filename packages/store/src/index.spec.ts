import { ChainIdNotAllowedError, createWeb3SolidStoreAndActions, MAX_SAFE_CHAIN_ID } from '.'
...
describe('#createWeb3SolidStoreAndActions', () => {
  test('throw on bad allowedChainIds', () => {
    expect(() => createWeb3SolidStoreAndActions([])).toThrow(`allowedChainIds is length 0`)
  })
  ...
  describe('#update', () => {
    ...
    test('errors on disallowed chainId', () => {
      const [store, actions] = createWeb3SolidStoreAndActions([2])
